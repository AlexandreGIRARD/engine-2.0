...
void Material::bind(const shared_program program)
{
    ...
    // Bind textures
    if (m_base_color)
        m_base_color->bind(program, 0, "base_color_texture");
    if (m_metallic_roughness)
        m_metallic_roughness->bind(program, 1, "metallic_roughness_texture");
    if (m_normal)
        m_normal->bind(program, 2, "normal_texture");
    if (m_emissive)
        m_emissive->bind(program, 3, "emissive_texture");
    if (m_occlusion)
        m_occlusion->bind(program, 4 ,"occlusion_texture");
}